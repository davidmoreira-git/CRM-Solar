...
const SMTP_FROM = process.env.SMTP_FROM || process.env.SMTP_USER || "CRM Solar <no-reply@crm-solar.local>";
...
const CAMPOS_CADASTRO_AUTOMATICO = [
  "cliente_nome",
  "documento",
  "cidade",
  "estado",
...
const DOCUMENTOS_ETAPA_DOCUMENTACAO = ["cnh_path", "talao_energia_path", "procuracao_path"];
const DOCUMENTOS_ETAPA_TRAMITACAO = ["boleto_trt_path", "diagrama_unifilar_path"];
const DOCUMENTOS_ETAPA_FINALIZADO = [
...
function hasEveryField(source, fields) {
  return fields.every((field) => isFilled(source?.[field]));
}

function isStatusAfter(candidate, current) {
  return STATUS_ORDEM[candidate] > STATUS_ORDEM[current];
}

function resolveAutomaticStatus(snapshot) {
  const cadastroCompleto = hasEveryField(snapshot.projeto, CAMPOS_CADASTRO_AUTOMATICO);
  const documentos = snapshot.documentos || {};
  let status = STATUS_PADRAO;

  if (
    cadastroCompleto &&
    snapshot.tem_proposta &&
    hasEveryField(documentos, DOCUMENTOS_ETAPA_DOCUMENTACAO)
  ) {
    status = "Tramitacao";
  }

  if (status === "Tramitacao" && hasEveryField(documentos, DOCUMENTOS_ETAPA_TRAMITACAO)) {
    status = "Analise";
  }

  if (status === "Analise" && isFilled(documentos.parecer_acesso_path)) {
    status = "Aprovado";
  }

  if (status === "Aprovado" && hasEveryField(documentos, DOCUMENTOS_ETAPA_FINALIZADO)) {
    status = "Finalizado";
  }
...
async function getAutomationSnapshot(queryable, projetoId) {
  const projetoResult = await queryable.query(
    `SELECT
       id,
       cliente_nome,
       documento,
  ...
  const documentosResult = await queryable.query(
    `SELECT *
  ...
  const propostaResult = await queryable.query(
    `SELECT EXISTS (
       SELECT 1
       FROM propostas
       WHERE projeto_id = $1
  ...
  return {
    projeto,
    documentos: documentosResult.rows[0] || null,
    tem_proposta: Boolean(propostaResult.rows[0]?.tem_proposta),
  };
}

async function advanceProjectStatusIfReady(queryable, projetoId, changedBy) {
  const snapshot = await getAutomationSnapshot(queryable, projetoId);

  if (!snapshot) {
    return null;
  }

  const currentStatus = snapshot.projeto.status_atual || STATUS_PADRAO;
  const nextStatus = resolveAutomaticStatus(snapshot);

  if (!isStatusAfter(nextStatus, currentStatus)) {
    return null;
  }
  ...
  const result = await queryable.query(
    `UPDATE projetos
     ...
     WHERE id = $2
     RETURNING id, cliente_nome, created_by, status_atual`,
    [nextStatus, projetoId]
  );

  await createStatusHistory(queryable, projetoId, currentStatus, nextStatus, changedBy);
...
async function sendMail({ to, subject, text }) {
  if (!mailTransporter) {
    console.warn("SMTP nao configurado. Email nao enviado:", subject);
    return { skipped: true };
  }
...
app.post("/propostas", authRequired, async (req, res) => {
  try {
    ...
    const automacao = result.rows[0].projeto_id
      ? await advanceProjectStatusIfReady(pool, result.rows[0].projeto_id, req.user.id)
      : null;
    await notifyAutomaticStatusChange(automacao, req.user);
...
app.put("/projetos/:id/dados", authRequired, async (req, res) => {
  ...
  try {
    ...
    const automacao = await advanceProjectStatusIfReady(pool, id, req.user.id);
    await notifyAutomaticStatusChange(automacao, req.user);
    const projetoAtualizado = automacao
      ? { ...result.rows[0], status_atual: automacao.status_novo }
      : result.rows[0];

    return res.json({
      message: "Projeto atualizado com sucesso.",
      projeto: mapProjeto(projetoAtualizado),
      automacao,
    });
  } catch (err) {
